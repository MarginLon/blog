...
module.exports = {
  ...
  env: {
    ...
    node: true, // 设置为所需检查的代码是在nodejs环境运行的
  ...
  rules: {
    ...
    'vue/max-attributes-per-line': [
      2,
      {
        ...
        multiline: {
          max: 1
          // allowFirstLine: false 有error，暂未查
        }
      }
    ], // 每行最大属性
    'vue/multi-word-component-names': 'off', // 组件名称为多词词组
    'vue/no-multiple-template-root': 'off', // This rule checks whether template contains single root element valid for Vue 2.
    ...
    'no-negated-in-lhs': 2, // no-unsafe-negation 禁止对关系运算符的左操作数使用否定操作符
    ...
    'no-spaced-func': 2, // func-call-spacing 要求或禁止在函数标识符和其调用之间有空格
    ...
    'use-isnan': 2, // 禁止比较时使用 NaN，只能用isNaN()
